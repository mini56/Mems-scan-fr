...
m_aboutString(QString("Une interface graphique pour le Rover Modular Engine Management System 1.6.   "))
{
...
void AboutBox::setupWidgets()
{
  ...
  m_url = new QLabel("Basé sur <a href=\"" + m_urlString + "\">" + "MEMS Gauge" + "</a> et <a href=\"" + m_urlString2 + "\">" + "MEMS rosco" + "</a>", this);
  m_url->setOpenExternalLinks(true);
  
  m_url2 = new QLabel("Outil d'analyse des fichiers journaux : <a href=\"" + m_urlString4 + "\">" + "MEMSAnalyser" + "</a>", this);
  m_url2->setOpenExternalLinks(true);

  m_urlLib = new QLabel("Utilise une version modifiée de <a href=\"" + m_urlLibString + "\">" + "Librosco" + "</a>.", this);
  m_urlLib->setOpenExternalLinks(true);
  
  m_urlDownload = new QLabel("Pour les mises à jour, le code source, les retours, etc. : <a href=\"" + m_urlString3 + "\">" + "MEMS-Scan" + "</a>", this);
  m_urlDownload->setOpenExternalLinks(true);

  m_ok = new QPushButton("Fermer", this);
